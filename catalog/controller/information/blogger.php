			...
			//if (!empty($setting['status']) && $setting['status']) {
				...
				$total_comments = $this->model_codezeel_blogger->getTotalBlogComments($module_blog['blogger_id']);
					...
					$module_blog_data[] = array(
						...
						'description' => utf8_substr(strip_tags(html_entity_decode($module_blog['description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '...',
						'date_added'  => date('M j, Y', strtotime($module_blog['date_added'])),
